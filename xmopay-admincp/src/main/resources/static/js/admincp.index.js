...
function readHistory(){
    ...
    if(h != '' && h != 'undefined'){
        ...
        if(arr.length){
            firstOpen = arr[arr.length - 1].split('-');//页面载入是默认打开的页面
            var tab = firstOpen[0];
            var item = firstOpen[1];
            if(typeof(menu[tab]) == 'undefined' || typeof(menu[tab]['children']) == 'undefined' || typeof(menu[tab]['children'][item]) == 'undefined'){
                firstOpen = "0dashboard-welcome".split('-');
            }
        }
    }
}
...
function addHistoryItem(tab, item){
    ...
    if($(id).length == 1){
        ...
        var close = $('<a href="javascript:;" class="close"><img src="/static/images/close.png" / ></a>');
        lnk.click(function(){
            openItem(item, tab);
        });
...
function loadSubmenu(){
    ...
    if(typeof(m.subtext) == 'undefined'){
        $('#submenuTitle').text(m.text);
    }else{
        $('#submenuTitle').text(m.subtext);
    }
...
function setWorkspace() {
    ...
    $('#workspace').height(wHeight - $('#header').height());
}
